...
conveyor_belt(BeltId, Truck) ->
  receive
    {package, PackageId, Size} ->
      ...
      Truck ! {package, PackageId, Size, self()},
      receive {ack,Truck} ->
        conveyor_belt(BeltId, Truck)
      end;
...
send_truck(TruckId) ->
  timer:sleep(rand:uniform(?TRUCK_SLEEP_TIME)+?TRUCK_SLEEP_TIME),%Min 2 seconds to max 4 seconds
  io:format("[Truck ~p] Arrived.~n", [TruckId]).
